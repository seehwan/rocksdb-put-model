...
#### 원본 파라미터 (p_stall=0.453, ρ_r=0.913, ρ_w=0.087)
```
입력 파라미터:
  압축률 (CR): 0.54
...
계산 결과:
  ...
  Harmonic mean 대역폭: 2251.3 MiB/s

바운드별 S_max:
  Write bound:        957.5 MiB/s
  Read bound:         2345.0 MiB/s
  Mixed (Harmonic):   879.6 MiB/s
  Level constraint:   61.6 MiB/s

최종 S_max:
  Feasible (stall 전): 61.6 MiB/s
  Final (stall 후):    33.7 MiB/s
  병목 지점: level_constraint
```

#### 조정된 파라미터 (p_stall=0.0, ρ_r=0.5, ρ_w=0.5)
```
입력 파라미터:
  압축률 (CR): 0.54
  Write Amplification (WA): 2.9
  ...
  읽기 비율 (ρ_r): 0.500
  쓰기 비율 (ρ_w): 0.500
  Stall 확률 (p_stall): 0.000

계산 결과:
  per-user 쓰기 요구량: 1.55
  per-user 읽기 요구량: 1.01
  Harmonic mean 대역폭: 1824.6 MiB/s

바운드별 S_max:
  Write bound:        957.5 MiB/s
  Read bound:         2345.0 MiB/s
  Mixed (Harmonic):   712.8 MiB/s
  Level constraint:   49.9 MiB/s

최종 S_max:
  Feasible (stall 전): 49.9 MiB/s
  Final (stall 후):    49.9 MiB/s
  병목 지점: level_constraint
```
...
| 항목 | v1 모델 | v2.1 (원본) | v2.1 (조정) | 실제 측정값 |
|------|---------|-------------|-------------|-------------|
| **S_max** | 582.0 MiB/s | 33.7 MiB/s | 49.9 MiB/s | 187.1 MiB/s |
| **오류율** | +211.1% | -82.0% | -73.3% | - |
| **병목 지점** | write | level_constraint | level_constraint | L2 |
| **정확도** | 낮음 | 중간 | 중간 | 기준 |
...
#### v2.1 모델
- **병목**: level_constraint (61.6 MiB/s)
