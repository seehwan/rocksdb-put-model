...
def parse_compaction_finish_line(line, timestamp):
    ...
    try:
        ...
        if files_match:
            ...
            # 레벨별 파일 수 분석
            level_analysis = {}
            for level, file_count in enumerate(files_per_level):
                if file_count > 0:
                    level_analysis[f'level_{level}'] = file_count
...
def analyze_compaction_flow(compaction_data):
    """Compaction Flow 분석"""
    print("📊 Compaction Flow 분석 중...")
    ...
    try:
        ...
        ax1.set_yticklabels([f'Level {l}' for l in base_level_counts.index])
        ...
        plt.tight_layout()
        plt.savefig('compaction_flow_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        print("✅ Compaction Flow 분석 시각화 저장 완료: compaction_flow_analysis.png")
        
        # 상세 분석 결과 출력
        print("\n📊 Compaction Flow 분석 결과:")
        ...
        # Compaction Flow 분석
        print(f"\n  Compaction Flow 분석:")
        print(f"    Level 0 → Level 1: {base_level_counts.get(0, 0):,}회")
        print(f"    Level 1 → Level 2: {base_level_counts.get(1, 0):,}회")
        print(f"    Level 2 → Level 3: {base_level_counts.get(2, 0):,}회")
        print(f"    Level 3 → Level 4: {base_level_counts.get(3, 0):,}회")
        print(f"    Level 4 → Level 5: {base_level_counts.get(4, 0):,}회")
...
def main():
    ...
    # Compaction Flow 분석
    analyze_compaction_flow(compaction_data)
    
    print("\n✅ Compaction Flow 분석 완료!")
