#!/usr/bin/env python3
"""
Phase-B Performance Pattern Analysis
Phase-B 성능 변화 패턴 분석
"""
...
class Phase_B_Performance_Pattern_Analyzer:
    def __init__(self, results_dir="results"):
        ...
        print("🚀 Phase-B Performance Pattern Analysis 시작")
        print("=" * 60)
    ...
    def analyze_performance_patterns(self):
        ...
        # 2. 성능 구간 분할
        performance_segments = self._segment_performance_by_changes(df, performance_changes)
        
        # 3. 구간별 특성 분석
        segment_characteristics = self._analyze_segment_characteristics(df, performance_segments)
        
        # 4. 성능 트렌드 분석
        performance_trends = self._analyze_performance_trends(df, performance_segments)
    ...
    def _analyze_performance_changes(self, df):
        ...
        # 큰 변화 지점 찾기 (표준편차의 2배 이상)
        threshold = change_stats['std_change'] * 2
    ...
    def _segment_performance_by_changes(self, df, performance_changes):
        """성능 변화를 기반으로 구간 분할"""
        print("📊 성능 변화 기반 구간 분할 중...")
        ...
        if len(large_changes) == 0:
            # 큰 변화가 없으면 단순히 3등분
            total_samples = len(df)
            segment_boundaries = [0, total_samples // 3, (total_samples * 2) // 3, total_samples]
            segments = {
            ...
            # 큰 변화 지점들을 기준으로 구간 분할
            change_points = sorted(large_changes.index.tolist())
    ...
    def _analyze_segment_characteristics(self, df, performance_segments):
        """구간별 특성 분석"""
        print("📊 구간별 특성 분석 중...")
        
        segment_characteristics = {}
        
        for phase_name, segment in performance_segments.items():
    ...
    def _analyze_performance_trends(self, df, performance_segments):
        """성능 트렌드 분석"""
        print("📊 성능 트렌드 분석 중...")
        ...
        # 구간 간 전환 분석
        phase_names = list(performance_segments.keys())
        for i in range(len(phase_names) - 1):
            current_phase = phase_names[i]
            next_phase = phase_names[i + 1]
            
            current_end = performance_segments[current_phase]['end']
            next_start = performance_segments[next_phase]['start']
        ...
        if len(phase_names) >= 2:
            ...
            first_avg = df.iloc[performance_segments[first_phase]['start']:performance_segments[first_phase]['end']]['interval_qps'].mean()
            last_avg = df.iloc[performance_segments[last_phase]['start']:performance_segments[last_phase]['end']]['interval_qps'].mean()
    ...
    def create_performance_pattern_visualization(self):
        """성능 패턴 시각화 생성"""
        print("📊 성능 패턴 시각화 생성 중...")
        ...
        df = self.phase_b_data['fillrandom_results']['dataframe']
        performance_segments = self.analysis_results['performance_segments']
        segment_characteristics = self.analysis_results['segment_characteristics']
        
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(18, 14))
        fig.suptitle('Phase-B Performance Pattern Analysis', fontsize=16, fontweight='bold')
        ...
        for i, (phase_name, segment) in enumerate(performance_segments.items()):
            start_idx = segment['start']
            end_idx = segment['end']
        ...
        ax1.set_title('Performance Curve with Segmented Phases')
        ax1.legend()
        ...
        ax2.set_title('Performance Statistics by Phase')
        ax2.set_xticks(x)
        ...
        ax3.set_title('Performance Variability by Phase')
        ax3.set_xticks(x)
        ...
        plt.tight_layout()
        plt.savefig(f"{self.results_dir}/phase_b_performance_pattern_analysis.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def save_results(self):
        """결과 저장"""
        print("💾 성능 패턴 분석 결과 저장 중...")
        
        # JSON 결과 저장
        try:
            with open(f"{self.results_dir}/phase_b_performance_pattern_analysis_results.json", 'w') as f:
        ...
        # Markdown 보고서 생성
        try:
            report_content = self._generate_performance_pattern_report()
            with open(f"{self.results_dir}/phase_b_performance_pattern_analysis_report.md", 'w') as f:
                f.write(report_content)
            print("✅ Markdown 보고서 생성 완료")
        except Exception as e:
            print(f"⚠️ Markdown 보고서 생성 실패: {e}")
    
    def _generate_performance_pattern_report(self):
        """성능 패턴 분석 보고서 생성"""
        report = f"""# Phase-B Performance Pattern Analysis
...
"""
        ...
        if 'performance_segments' in self.analysis_results:
            segments = self.analysis_results['performance_segments']
            report += f"""
...
### 1. Natural Performance Phases
- **Change-based Segmentation**: Segments identified based on actual performance changes
- **Performance Variability**: Different phases show different variability patterns
- **Trend Analysis**: Each phase has distinct performance trends
...
- **Accurate Phase Definition**: Natural performance phases for better model training
- **Phase-specific Modeling**: Different models for different performance phases
- **Trend-based Predictions**: Trend analysis for better performance prediction

## Visualization
![Phase-B Performance Pattern Analysis](phase_b_performance_pattern_analysis.png)
...
"""
    ...
    def run_analysis(self):
        ...
        # 2. 시각화 생성
        self.create_performance_pattern_visualization()
        
        # 3. 결과 저장
        self.save_results()
        
        print("=" * 60)
        print("✅ Phase-B 성능 패턴 분석 완료!")
