#!/usr/bin/env python3
"""
Phase-B RocksDB Characteristics Analysis
...
class Phase_B_RocksDB_Characteristics_Analyzer:
    def __init__(self, results_dir="results"):
        ...
        print("🚀 Phase-B RocksDB Characteristics Analysis 시작")
        print("=" * 60)
    ...
    def analyze_rocksdb_operational_phases(self):
        ...
        if not self.phase_b_data:
            print("⚠️ Phase-B 데이터가 없습니다.")
            return None
        
        # Phase-B 데이터에서 시간별 성능 데이터 추출
        if 'results' in self.phase_b_data and len(self.phase_b_data['results']) > 0:
            results = self.phase_b_data['results']
            
            # 시간별 데이터 정리
            time_series_data = []
            for result in results:
                if 'time' in result and 'ops_per_sec' in result:
                    time_series_data.append({
                        'time': result['time'],
                        'ops_per_sec': result['ops_per_sec'],
                        'cumulative_ops': result.get('cumulative_ops', 0)
                    })
            
            if not time_series_data:
                print("⚠️ 시간별 성능 데이터가 없습니다.")
                return None
            ...
            # RocksDB 운영 구간 분석
            operational_phases = self._identify_rocksdb_operational_phases(df)
            
            print("✅ RocksDB 운영 구간 분석 완료")
            return operational_phases
        else:
            print("⚠️ Phase-B 결과 데이터가 없습니다.")
            return None
    
    def _identify_rocksdb_operational_phases(self, df):
        """RocksDB 운영 구간 식별"""
        print("📊 RocksDB 운영 구간 식별 중...")
        ...
        operational_phases['initial_phase']['end_time'] = df['time'].iloc[initial_end_idx]
        operational_phases['middle_phase']['start_time'] = df['time'].iloc[initial_end_idx]
        
        # 중기 구간: 20% ~ 80% (컴팩션 활동 구간)
        middle_end_idx = int(total_samples * 0.8)
        operational_phases['middle_phase']['end_time'] = df['time'].iloc[middle_end_idx]
        operational_phases['final_phase']['start_time'] = df['time'].iloc[middle_end_idx]
...
- **Time Range**: {phase_info['start_time']} - {phase_info['end_time']}
