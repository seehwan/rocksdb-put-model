...
v4.2 FillRandom Enhanced 모델을 사용한 Phase-C 분석
"""
...
class Phase_C_V4_2_Analyzer:
    def __init__(self, results_dir="results"):
        ...
        print("🚀 Phase-C Analysis with V4.2 FillRandom Enhanced Model 시작")
        print("=" * 60)
    ...
    def _load_phase_b_data(self):
        ...
        # Phase-B 결과 파일들
        phase_b_files = [
            '/home/sslab/rocksdb-put-model/experiments/2025-09-12/phase-b/results/phase_b_analysis_results.json',
            '/home/sslab/rocksdb-put-model/experiments/2025-09-12/phase-b/results/hourly_level_io_analysis.json',
            '/home/sslab/rocksdb-put-model/experiments/2025-09-12/phase-b/results/flush_compaction_throughput_analysis.json'
        ]
        ...
        for file_path in phase_b_files:
            if os.path.exists(file_path):
                try:
                    ...
                    phase_b_data[file_name] = data
                    print(f"   ✅ {file_name} 로드 완료")
                except Exception as e:
                    print(f"   ⚠️ {file_path} 로드 실패: {e}")
        
        print(f"✅ Phase-B 데이터 로드 완료: {len(phase_b_data)}개 파일")
        return phase_b_data
    ...
    def analyze_phase_c_with_v4_2(self):
        """v4.2 모델을 사용한 Phase-C 분석"""
        ...
        phase_performance = self.v4_2_model_results.get('phase_performance', {})
        
        # Phase-B 실제 성능 데이터
        phase_b_actual_performance = self._extract_phase_b_actual_performance()
        
        # v4.2 모델 정확도 분석
        model_accuracy_analysis = self._analyze_v4_2_model_accuracy(v4_2_predictions, phase_b_actual_performance)
        
        # 시기별 성능 비교 분석
        temporal_performance_comparison = self._analyze_temporal_performance_comparison(v4_2_predictions, phase_b_actual_performance)
        
        # FillRandom 워크로드 특성 분석
        fillrandom_workload_analysis = self._analyze_fillrandom_workload_characteristics(v4_2_predictions, phase_b_actual_performance)
        
        # Compaction 효율성 분석
        compaction_efficiency_analysis = self._analyze_compaction_efficiency(v4_2_predictions, phase_b_actual_performance)
        ...
        self.phase_c_analysis = {
            ...
            'phase_b_actual_performance': phase_b_actual_performance,
    ...
    def _extract_phase_b_actual_performance(self):
        """Phase-B 실제 성능 데이터 추출"""
        print("📊 Phase-B 실제 성능 데이터 추출 중...")
        
        # Phase-B에서 추출할 수 있는 실제 성능 지표들
        actual_performance = {
            'hourly_throughput': {},
            'level_io_characteristics': {},
            'flush_compaction_throughput': {},
            'performance_degradation': {}
        }
        
        # Phase-B 분석 결과에서 실제 성능 데이터 추출
        if 'phase_b_analysis_results' in self.phase_b_data:
            phase_b_results = self.phase_b_data['phase_b_analysis_results']
            
            # 실제 QPS 데이터
            if 'summary' in phase_b_results:
                summary = phase_b_results['summary']
                actual_performance['hourly_throughput'] = {
                    'avg_qps': summary.get('avg_qps', 0),
                    'max_qps': summary.get('max_qps', 0),
                    'min_qps': summary.get('min_qps', 0)
                }
            
            # 성능 열화 분석
            if 'performance_degradation' in phase_b_results:
                degradation = phase_b_results['performance_degradation']
                actual_performance['performance_degradation'] = {
                    'degradation_rate': degradation.get('degradation_rate', 0),
                    'initial_performance': degradation.get('initial_performance', 0),
                    'final_performance': degradation.get('final_performance', 0)
                }
        
        # 시간별 레벨 I/O 특성
        if 'hourly_level_io_analysis' in self.phase_b_data:
            level_io_data = self.phase_b_data['hourly_level_io_analysis']
            actual_performance['level_io_characteristics'] = level_io_data
        
        # Flush/Compaction 처리량
        if 'flush_compaction_throughput_analysis' in self.phase_b_data:
            throughput_data = self.phase_b_data['flush_compaction_throughput_analysis']
            actual_performance['flush_compaction_throughput'] = throughput_data
        
        print(f"✅ Phase-B 실제 성능 데이터 추출 완료")
        return actual_performance
    
    def _analyze_v4_2_model_accuracy(self, v4_2_predictions, phase_b_actual_performance):
        """v4.2 모델 정확도 분석"""
        ...
        # Phase-B 실제 성능 데이터
        actual_qps = phase_b_actual_performance.get('hourly_throughput', {}).get('avg_qps', 0)
    ...
    def _analyze_temporal_performance_comparison(self, v4_2_predictions, phase_b_actual_performance):
        """시기별 성능 비교 분석"""
    ...
    def _analyze_fillrandom_workload_characteristics(self, v4_2_predictions, phase_b_actual_performance):
        """FillRandom 워크로드 특성 분석"""
    ...
    def _analyze_compaction_efficiency(self, v4_2_predictions, phase_b_actual_performance):
        """Compaction 효율성 분석"""
    ...
    def create_phase_c_v4_2_visualization(self):
        """Phase-C v4.2 모델 분석 시각화 생성"""
        print("📊 Phase-C v4.2 모델 분석 시각화 생성 중...")
        
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(18, 14))
        fig.suptitle('Phase-C Analysis with V4.2 FillRandom Enhanced Model', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(f"{self.results_dir}/phase_c_v4_2_analysis.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def save_results(self):
        ...
        # JSON 결과 저장
        try:
            with open(f"{self.results_dir}/phase_c_v4_2_analysis_results.json", 'w') as f:
        ...
        # Markdown 보고서 생성
        try:
            report_content = self._generate_phase_c_v4_2_report()
            with open(f"{self.results_dir}/phase_c_v4_2_analysis_report.md", 'w') as f:
                f.write(report_content)
            print("✅ Markdown 보고서 생성 완료")
        except Exception as e:
            print(f"⚠️ Markdown 보고서 생성 실패: {e}")
    
    def _generate_phase_c_v4_2_report(self):
        """Phase-C v4.2 모델 분석 보고서 생성"""
        report = f"""# Phase-C Analysis with V4.2 FillRandom Enhanced Model
...
"""
    ...
    def run_analysis(self):
        """전체 분석 실행"""
        print("🚀 Phase-C v4.2 모델 분석 시작")
        print("=" * 60)
        
        self.analyze_phase_c_with_v4_2()
        self.create_phase_c_v4_2_visualization()
        self.save_results()
        
        print("=" * 60)
        print("✅ Phase-C v4.2 모델 분석 완료!")
        print(f"📊 결과 저장 위치: {self.results_dir}")

if __name__ == "__main__":
    analyzer = Phase_C_V4_2_Analyzer()
